  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground mt-1">Download CrashPlan clients and tools</p>
      </div>

      <div className="bg-card rounded-lg shadow-sm border border-border p-6">
        <h3 className="text-lg font-semibold text-foreground mb-4">Available Downloads</h3>
        <div className="space-y-4">
          {downloads.map((download, index) => {
            ...
            return (
              <div
                key={index}
                className="flex items-center justify-between p-4 border border-border rounded-lg hover:border-brand-blue transition-colors"
              >
                <div className="flex items-center space-x-4">
                  <Icon className={`${download.iconColor} w-8 h-8`} />
                  <div>
                    <h4 className="font-medium text-foreground">{download.name}</h4>
                    <p className="text-sm text-muted-foreground">{download.version}</p>
                  </div>
                </div>
